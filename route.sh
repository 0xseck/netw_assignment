...
#TCP_send(){
#set -xv
 #   var=`echo ${1:-$(</dev/stdin)}`  ---> Gets stuck if buffer is empty 
...
#   tee -a >(TCP_send) | \
   ...
   if test ! $? -eq "0"
   then
        echo "{TCP}Can't connect to A4"  >> route.txt
    fi
...
UDP_do(){
   ...
   xargs -d'\n' -IMSG sh -c  " echo 'ROUTE-->UDP: [MSG] {timestamp:\"'\`date +%s%3N\`'\"}' >> route.txt " ) 2>/dev/null 

    if test ! $? -eq "0"
    then
        echo "{UDP}Can't connect to A3" >> route.txt
    fi
}
...
UDP_do &
TCP_do & 
while :; do echo -ne "\e[33m\e[K\rListening...Use ctrl+c to exit\e[0m";sleep 2;done
